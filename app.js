...
const artPage=`<main id="main"><div class="wrap"><header class="pagehead"><p class="eyebrow">Theology & Liturgy</p><h1>Religious Art</h1><p class="lead">A searchable repository of photographs of stained glass and other religious artwork encountered in churches I have visited.</p></header>${subnav}<section class="art-controls" aria-label="Filter religious artwork"><div class="art-search"><label for="art-search">Search</label><input id="art-search" type="search" placeholder="Title, church, topic, saint…"></div><div><label for="art-church">Church</label><select id="art-church"><option value="">All churches</option></select></div><div><label for="art-topic">Topic</label><select id="art-topic"><option value="">All topics</option></select></div><div><label for="art-season">Season</label><select id="art-season"><option value="">All seasons</option></select></div><div><label for="art-figure">Figure or saint</label><select id="art-figure"><option value="">All figures</option></select></div><div><label for="art-type">Artwork type</label><select id="art-type"><option value="">All types</option></select></div><button class="button secondary" id="art-clear" type="button">Clear filters</button></section><div class="art-results"><p id="art-count" aria-live="polite"></p></div><section id="art-gallery" class="art-gallery" aria-label="Religious art gallery"></section><dialog id="art-dialog"><button id="art-dialog-close" class="dialog-close" type="button" aria-label="Close image">×</button><img id="art-dialog-image" alt=""><div id="art-dialog-caption"></div></dialog></div></main>`;
const simple={
...
if(path==='religious-art.html'){
  const controls={search:document.querySelector('#art-search'),church:document.querySelector('#art-church'),topic:document.querySelector('#art-topic'),season:document.querySelector('#art-season'),figure:document.querySelector('#art-figure'),type:document.querySelector('#art-type')};
  ...
  addOptions(controls.topic,religiousArt.flatMap(item=>item.tags||[]));
  addOptions(controls.season,religiousArt.flatMap(item=>item.seasons||[]));
  addOptions(controls.figure,religiousArt.flatMap(item=>item.figures||[]));
  ...
  const renderGallery=()=>{
    ...
    const matches=religiousArt.filter(item=>(!query||searchable(item).includes(query))&&(!controls.church.value||item.church===controls.church.value)&&(!controls.topic.value||(item.tags||[]).includes(controls.topic.value))&&(!controls.season.value||(item.seasons||[]).includes(controls.season.value))&&(!controls.figure.value||(item.figures||[]).includes(controls.figure.value))&&(!controls.type.value||item.type===controls.type.value));
    count.textContent=`Showing ${matches.length} of ${religiousArt.length} ${religiousArt.length===1?'work':'works'}`;
    gallery.innerHTML=matches.map(item=>`<article class="art-card"><button class="art-image-button" type="button" data-index="${religiousArt.indexOf(item)}" aria-label="View ${item.title} at full size"><img src="${item.image}" alt="${item.alt}" loading="lazy"></button><div class="art-card-body"><p class="eyebrow">${item.type||'Religious art'}</p><h2>${item.title}</h2><p><strong>${item.church}</strong>${item.location?`<br>${item.location}`:''}</p>${tags([...(item.seasons||[]),...(item.figures||[])].filter(Boolean))}</div></article>`).join('');
